...
const ListLab = ({ courseData, searchValue }) => {
  ...
  const [hideCompleted, setHideCompleted] = useState(false) // Default: Menampilkan semua data
  const [filteredData, setFilteredData] = useState(courseData) // Awalnya semua data ditampilkan
  ...
  useEffect(() => {
    ...
    // Jika hideCompleted true, filter data yang selesai: false
    if (hideCompleted) {
    ...
    // Filter berdasarkan nilai search jika ada
    if (searchValue) {
      newData = newData.filter(category => category.namaLab.toLowerCase().includes(searchValue.toLowerCase()));
    }

    setFilteredData(newData); // Setel data yang telah difilter
